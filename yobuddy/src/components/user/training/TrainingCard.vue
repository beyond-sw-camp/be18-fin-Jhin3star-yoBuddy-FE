    ...
    <div class="card-body">
      <div class="card-header-row">
        ...
        <div class="status-box-inline">
          <div class="status-pill" :class="statusClass(displayStatus)">{{ statusLabel(displayStatus) }}</div>
        </div>
      </div>
      ...
      <div class="info-action-wrap">
        <div class="info-row">
          ...
          <div class="action-row">
            <button v-if="training?.type === 'ONLINE' && !isCompleteLike" class="btn-primary" @click.stop="openUrl">수강하기</button>
          </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    displayStatus() {
      return this.deriveStatus(this.training)
    },
    isCompleteLike() {
      const up = String(this.displayStatus || '').toUpperCase()
      return up === 'COMPLETED' || up === 'TARDY'
    }
  },
  methods: {
    ...
    deriveStatus(t){
      const base = String((t && t.status) || '').toUpperCase()
      if (base === 'TARDY') return 'TARDY'
      if (!t) return base
      if (t.type === 'ONLINE') {
        const endDate = this.toDate(t.endDate)
        const completion = this.toDate(this.getCompletionDate(t))
        if (endDate && completion && completion > endDate) return 'TARDY'
      }
      return base
    },
    getCompletionDate(t){
      if(!t) return null
      if(Array.isArray(t.certificateFiles) && t.certificateFiles[0]?.uploadedAt) return t.certificateFiles[0].uploadedAt
    ...
    statusLabel(s){ if(!s) return '상태 없음'; const up=String(s).toUpperCase(); if(up==='PENDING') return '예정'; if(up==='IN_PROGRESS') return '진행 중'; if(up==='COMPLETED') return '완료'; if(up==='MISSED') return '미이수'; if(up==='TARDY') return '지각'; return s },
    statusClass(s){ const up=String(s||'').toUpperCase(); if(up==='PENDING') return 'pill-pending'; if(up==='COMPLETED') return 'pill-complete'; if(up==='IN_PROGRESS') return 'pill-active'; if(up==='MISSED'||up==='TARDY') return 'pill-missed'; return 'pill-default' },
    getDisplayDate(t){
      if(!t) return ''
      const st = this.deriveStatus(t)
      const completion = this.getCompletionDate(t)
      if((st === 'COMPLETED' || st === 'TARDY') && completion) return this.formatDate(completion)
